#!/usr/bin/env python3
"""
recon.py - subdomain enumeration & attack-surface mapping pipeline  v1.4
...
v1.4 adds:
    - certspotter   (CT aggregator, no API key — redundant with crt.sh)
    - OTX           (AlienVault passive DNS, no API key)
    - TLS SAN enum  (extract SAN names from live host certificates, --tls-san)
    - ASN expansion (reverse-DNS on ASN prefixes, opt-in --asn)

Pipeline per target:
    root domain discovery (whois / whoxy)
      -> passive enumeration   (subfinder, crt.sh, certspotter, Chaos,
                                 VirusTotal, OTX, gau/waybackurls,
                                 assetfinder, github-subdomains, amass)
      -> optional TLS SAN enum (scrape certificates of live hosts)
      -> optional ASN expansion (reverse-DNS on ASN IP ranges)
...
crt.sh/certspotter + a threaded resolver keep it useful with none of the Go tools.

Usage:
    python3 recon_v1.4.py -t example.com -o output.txt
    python3 recon_v1.4.py -T targets.txt -o output.txt --vhost --tls-san --recursive
...
# --------------------------------------------------------------------------- #
# TLS SAN enumeration (v1.4)
# --------------------------------------------------------------------------- #
def tls_san_from_host(host, port=443, timeout=5):
    """Extract DNS SAN entries from a single host's TLS certificate."""
    names = set()
    try:
...
def tls_san_enum(hosts, domain, threads=20):
    """For each live host, scrape its TLS cert and extract in-scope SAN names."""
    found = set()
    hosts = sorted(set(hosts))
    if not hosts:
        return found
    with ThreadPoolExecutor(max_workers=threads) as ex:
        futs = {ex.submit(tls_san_from_host, h): h for h in hosts}
        for fut in as_completed(futs):
            ...
            try:
                ...
                continue
            for name in sans:
                if in_scope(name, domain) and name not in found:
                    found.add(name)
                    print(f"    {name}  (SAN from {h})")
    return found


# --------------------------------------------------------------------------- #
# ASN expansion (v1.4, opt-in --asn)
# --------------------------------------------------------------------------- #
def asn_from_ip(ip, timeout=15):
    """Resolve an IP to an ASN via bgpview.io. Returns (asn, asn_name) or (None, None)."""
    url = f"https://api.bgpview.io/ip/{urllib.parse.quote(ip)}"
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "recon.py/2.2"})
...
def asn_prefixes(asn, timeout=15):
    """Get IPv4 prefixes for an ASN via bgpview.io. Returns list of prefix strings."""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            data = json.loads(resp.read().decode("utf-8", "replace"))
        pfx_list = []
        for pfx in (data.get("data") or {}).get("ipv4_prefixes", []):
            p = pfx.get("prefix", "")
            if p:
                pfx_list.append(p)
        return pfx_list
...
def reverse_dns_lookup(ip, timeout=4):
    """Single reverse-DNS lookup returning the PTR hostname or None."""
    try:
...
def asn_expand(domain, ips, threads=20):
    """
    ASN-based subdomain discovery (opt-in via --asn).

    For each unique IP: look up the ASN, fetch the ASN's IPv4 prefixes, then
    reverse-DNS a small random sample of IPs in each prefix and keep in-scope
    results. Only meaningful for targets with their own IP space (not behind
    shared CDN/WAF like Cloudflare).
    """
    found = set()
    if not ips:
        return found
    ...
    info(f"ASN expansion: resolving ASN for {len(unique_ips)} IP(s) ...")
    for ip in unique_ips:
        asn, name = asn_from_ip(ip)
        if asn:
            asns.setdefault(asn, {"name": name, "ips": []})["ips"].append(ip)
            print(f"    {ip} -> AS{asn} ({name or 'unknown'})")

    if not asns:
        info("no ASN data found - ASN expansion yielded nothing")
        return found

    all_prefixes = set()
    for asn, asn_info in asns.items():
        info(f"AS{asn} ({asn_info['name']}): fetching IP prefixes ...")
        for p in asn_prefixes(asn):
            all_prefixes.add(p)
            print(f"    {p}")

    if not all_prefixes:
        info("no prefixes found for any ASN")
        return found

    # Light sampling: ~10 IPs per /24 equivalent (avoids scanning the full range).
    info(f"sampling IPs from {len(all_prefixes)} prefix(es) for reverse-DNS ...")
    sample_ips = set()
    for pfx in all_prefixes:
        parts = pfx.split("/")
        if len(parts) != 2:
            continue
        base, mask = parts[0], int(parts[1])
        octets = base.split(".")
        if len(octets) != 4:
            continue
        try:
            net = (int(octets[0]) << 24 | int(octets[1]) << 16
        ...
        if host_bits <= 0:
            continue
        total = 1 << host_bits
        sample_count = min(max(10, total // 256), 256)
        for _ in range(sample_count):
            offset = random.randint(1, total - 2)
            ip_int = (net & ~((1 << host_bits) - 1)) + offset
            sample_ips.add(f"{(ip_int >> 24) & 0xff}.{(ip_int >> 16) & 0xff}."
                           f"{(ip_int >> 8) & 0xff}.{ip_int & 0xff}")

    info(f"reverse-DNS on {len(sample_ips)} sampled IP(s) ...")
    with ThreadPoolExecutor(max_workers=threads) as ex:
        futs = {ex.submit(reverse_dns_lookup, ip): ip for ip in sample_ips}
        for fut in as_completed(futs):
            ip = futs[fut]
            try:
                ptr = fut.result()
            ...
            if not ptr:
                continue
            print(f"    {ptr}  (PTR of {ip})")
            if in_scope(ptr, domain):
                found.add(ptr)
    return found
...
def certspotter_source(domain, retries=3):
    """CertSpotter CT lookup (alternative to crt.sh, no API key needed)."""
    found = set()
...
def otx_source(domain, retries=2):
    """
    AlienVault OTX passive DNS (no API key required).
    Returns (set_of_hosts, status) in {"ok", "error"}.
    """
    found = set()
    ...
    for attempt in range(retries):
        try:
            ...
            for entry in data.get("passive_dns", []):
                h = str(entry.get("hostname", "")).strip().lower().strip(".")
            ...
            if attempt < retries - 1:
                time.sleep(2 * (attempt + 1))
                continue
            warn(f"OTX query failed after {retries} tries: {e}")
    return found, "error"
...
def passive_enum(domain, amass=False, github_token=None, keys=None):
    ...
    info("querying CertSpotter (CT aggregator) ...")
    add(certspotter_source(domain), "certspotter")
    ...
    info("querying AlienVault OTX (passive DNS, no key needed) ...")
    names, st = otx_source(domain)
...
def run_pipeline(domain, args, ctx, queue, visited):
    ...
    # Phase 4.5 - TLS SAN enumeration (v1.4, opt-in)
    if not args.passive_only and args.tls_san and resolved:
        phase(f"TLS SAN enumeration - {domain}")
        tls_found = tls_san_enum(resolved, domain, args.threads)
        tag(tls_found, "tls-san")
        new_san = tls_found - set(sources)
        if new_san:
            good(f"{len(new_san)} new names from TLS certificates")
            # Try resolving the newly discovered SAN names.
            san_resolved = resolve_hosts(new_san, bulk, trusted, args.threads, wildcard_ips)
            resolved |= san_resolved
            result["unresolved"] = set(sources) - resolved
        good(f"{len(tls_found)} SAN-derived names ({len(tls_found & resolved)} live)")

    # Phase 4.6 - ASN expansion (v1.4, opt-in --asn)
    if not args.passive_only and args.asn and resolved:
        phase(f"ASN-based expansion - {domain}")
        asn_ips = ips_from_hosts(resolved)
        if asn_ips:
            asn_found = asn_expand(domain, asn_ips, args.threads)
            tag(asn_found, "asn-expand")
            new_asn = asn_found - set(sources)
            if new_asn:
                good(f"{len(new_asn)} new names from ASN reverse-DNS")
                asn_resolved = resolve_hosts(new_asn, bulk, trusted, args.threads, wildcard_ips)
                resolved |= asn_resolved
                result["unresolved"] = set(sources) - resolved
            good(f"{len(asn_found)} ASN-derived names ({len(asn_found & resolved)} live)")
        else:
            info("no IPs to feed into ASN expansion")
...
def parse_args():
    ...
    p.add_argument("--tls-san", action="store_true",
                   help="scrape TLS cert SAN entries from live hosts for extra subdomains (v1.4)")
    p.add_argument("--asn", action="store_true",
                   help="ASN-based expansion: reverse-DNS on the target IP's ASN blocks "
                        "(only meaningful for self-hosted infrastructure, v1.4)")
