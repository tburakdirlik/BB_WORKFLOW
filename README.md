...
> **Warning:** Only test assets you are explicitly authorized to assess.
...
To let Claude do it (and review each edit), start `claude` from the repo root and paste everything between `PROMPT START` and `PROMPT END` below. It makes the same file changes, described step by step in plain text, so it pastes cleanly.
